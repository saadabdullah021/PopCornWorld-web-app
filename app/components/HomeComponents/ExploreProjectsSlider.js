...
const ExploreProjectsSlider = ({ campaigns, campaignsLoading, campaignsError, globalSettings }) => {
  ...
  // Format author name (e.g., "Saad A.")
  const formatAuthorName = useCallback((name) => {
  ...
  // Transform campaigns
  const transformCampaigns = useCallback((campaignsData) => {
    ...
    return campaignsData.map(campaign => {
      ...
      const percentage = Math.min(Math.floor((collectedAmount / raiseAmount) * 100), 100);
      ...
      return {
        ...
        author: formatAuthorName(campaign.fundraiser?.name),
        ...
        image: campaign.galleries?.[0]?.image
          ...
          : campaign.campaign_image?.trim()
            ? (campaign.campaign_image.trim().startsWith('uploads')
              ...
              : campaign.campaign_image.trim())
            : '/pop_packet.png',
        raised: `${currency}${formatNumber(collectedAmount)}`,
  ...
  }, [getCurrency, formatAuthorName]);

  // Memoize transformed campaigns
  const originalProjects = useMemo(() => transformCampaigns(campaigns), [campaigns, transformCampaigns]);
  ...
  const projects = useMemo(() => shouldShowSlider ? [
    ...originalProjects.slice(-itemsPerView),
    ...originalProjects,
    ...originalProjects.slice(0, itemsPerView),
  ] : originalProjects, [originalProjects, shouldShowSlider, itemsPerView]);

  // Set initial slide
  useEffect(() => {
    setCurrentSlide(shouldShowSlider ? itemsPerView : 0);
  }, [shouldShowSlider, itemsPerView]);
  ...
  }, [currentSlide, originalProjects.length, itemsPerView]);
  ...
  const imageLoader = useCallback(({ src, width }) => `${src}?w=${width}&q=75`, []);

  const ProjectCard = ({ project }) => (
    <div
      className="bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 group cursor-pointer h-[450px] transform hover:-translate-y-1"
      onClick={() => (window.location.href = `/campaigns/${project.slug}`)}
      role="button"
      tabIndex={0}
      onKeyDown={(e) => e.key === 'Enter' && (window.location.href = `/campaigns/${project.slug}`)}
    >
      <div className="relative h-48 overflow-hidden">
        ...
        />
        <div
          className="absolute top-2 right-2 bg-white/80 backdrop-blur-sm rounded-full p-2 shadow-md hover:bg-white transition-colors cursor-pointer group/share"
          onClick={(e) => {
            e.stopPropagation();
          ...
          onKeyDown={(e) => e.key === 'Enter' && (setShareProject(project), setOpenShare(true))}
        >
          <svg
  ...
  <hr className="w-full border-gray-300 border-t" />
        ...
        <div className="mb-4">
          <div className="flex justify-between items-center mb-2">
            <span className="text-sm font-semibold text-gray-700">Raised: {project.raised}</span>
            <span className="text-sm font-bold text-gray-800">{project.percentage}%</span>
          </div>
        ...
        {shouldShowSlider ? (
          <>
            ...
            <div className="flex items-center justify-center gap-4 mt-8">
              <button
                onClick={prevSlide}
                className="w-12 h-12 bg-white/90 hover:bg-white rounded-full shadow-lg flex items-center justify-center transition-all duration-200 hover:scale-105 disabled:opacity-50"
