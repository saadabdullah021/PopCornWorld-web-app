...
const MinimalSharePopup = ({ isOpen, onClose, url, title }) => {
  ...
  const handleShare = (option) => {
    if (navigator.share && option.name !== 'Email') {
      ...
      if (option.name === 'Instagram') {
        // For Instagram, we'll copy the link instead since direct sharing isn't supported
        copyToClipboard();
        toast('Link copied! Open Instagram to share', {
          position: 'bottom-center',
          duration: 2000,
          style: {
            background: '#E1306C',
      ...
      <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fadeIn">
        <div className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl transform animate-scaleIn">
          ...
          <div className="relative p-6 pb-4">
            <button
              onClick={onClose}
              className="absolute top-4 right-4 text-gray-400 hover:text-gray-600 p-2 rounded-full hover:bg-gray-100 transition-colors duration-200"
            >
            ...
            <div className="flex flex-col sm:flex-row items-start  sm:items-center space-x-3  lg:mb-4 border-b pb-4 border-gray-200">
              ...
              <div>
                <h3 className="text-2xl font-semibold text-black">Support by sharing</h3>
          ...
          <div className="px-6 pb-6 flex items-center justify-between flex-col sm:flex-row space-y-6 sm:space-y-0 sm:space-x-6">
            ...
            <div className="bg-gray-100 rounded-xl p-3">
              <div className="flex items-center  space-x-3">
                ...
                <button
                  ...
                  className="bg-black hover:bg-black cursor-pointer text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors duration-200 flex items-center space-x-2"
                >
            ...
            <div className="flex justify-start flex-wrap sm:flex-nowrap space-y-4 space-x-4 py-4">
              {shareOptions.map((option) => (
                <button
                  ...
                  className={`${option.color} text-white cursor-pointer w-12 h-12 flex items-center justify-center rounded-full hover:shadow-xl transition-all duration-200 transform hover:scale-105 shadow-2xl focus:outline-none `}
                  title={option.name}
                >
                  {option.icon}
                </button>
            ...
            {/* QR Code Section - Smaller and Optional */}
...
export default MinimalSharePopup;